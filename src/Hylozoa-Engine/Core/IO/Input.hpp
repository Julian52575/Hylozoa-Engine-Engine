...
class Input {
  public:
    Input(entt::registry &registry)
        : m_registry(registry),
          m_inputState(
              registry.ctx().get<Components::HylozoaInternal::InputState>()),
          m_mouseState(
              registry.ctx().get<Components::HylozoaInternal::MouseState>()),
          m_engineEvents(
              registry.ctx().get<Components::HylozoaInternal::EngineEvents>()) {
    }
    ~Input() = default;
