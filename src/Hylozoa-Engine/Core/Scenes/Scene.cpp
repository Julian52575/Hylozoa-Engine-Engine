...
void Scene::destroyScene(entt::registry &registry) {
    ...
    for (auto entity : view) {
        ...
        if (sceneTag.id == m_id) {
            ...
            if (Hylozoa::Settings::getInstance().getSettings().verbose) {
                std::cout << "Destroyed entity with Name '"
                          << toDelete.getName() << "' from scene '" << m_name
                          << "' (scene ID: " << m_id << ")." << std::endl;
            }
            toDelete.destroy();
        }
    }
...
Entity SceneManager::instantiatePrefab(const std::string &prefabPath,
...
void SceneManager::destroyScene(std::string_view name) {
    ...
    throw std::runtime_error(
        "SceneManager::destroyScene (name) - Scene not found: " +
        std::string(name));
}
...
void SceneManager::loadScene(std::string_view name) {
    ...
    throw std::runtime_error(
        "SceneManager::loadScene (name) - Scene not found: " +
        std::string(name));
}
