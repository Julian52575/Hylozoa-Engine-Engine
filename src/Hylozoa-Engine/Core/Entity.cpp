...
Entity Entity::child_of(Entity parent, Engine& engine)
{
    ...
    if (registry.valid(m_entity) && registry.valid(parent.get_internal_entity())) {
        registry.emplace_or_replace<Parent>(m_entity, Parent{parent.get_internal_entity()});
    } else {
        std::cerr << "[Entity] Warning: Trying to set parent on invalid entity." << std::endl;
    }
    return *this;
}
