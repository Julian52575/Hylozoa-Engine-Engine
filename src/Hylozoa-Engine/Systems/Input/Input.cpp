...
void Input::onStart() { std::cout << "[" << this->_name << "] Start\n"; }
...
void Input::onUpdate(float) {
  while (SDL_PollEvent(&event)) {
    ...
    case SDL_EVENT_MOUSE_BUTTON_DOWN:
      std::cout << "Mouse button pressed at (" << event.button.x << ", "
                << event.button.y << ")\n";
      break;
...
void Input::onEnd() { std::cout << "[" << this->_name << "] End\n"; }

const std::string &Input::getName() const { return this->_name; }
