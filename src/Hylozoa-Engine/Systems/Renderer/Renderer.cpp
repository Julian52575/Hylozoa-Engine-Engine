...
void Renderer::onUpdate(float deltaTime) {
    ...
    if (!this->_registry) {
        SDL_SetRenderDrawColor(renderer.get(), 148, 0, 211, 255); // Ugly debug purple
    ...
    for (auto e : camView) cameras.push_back(e);

    // sort cameras by order (low -> high)
    
    std::sort(cameras.begin(), cameras.end(), [&](entt::entity a, entt::entity b) {
    ...
    for (auto camEntity : cameras) {
        const Hylozoa::Components::Camera &cam = camView.get<Hylozoa::Components::Camera>(camEntity);
        const Hylozoa::Components::WorldTransform &camTransform = camView.get<Hylozoa::Components::WorldTransform>(camEntity);
...
void Renderer::renderSingleCamera(const Hylozoa::Components::Camera& camera, const Hylozoa::Components::WorldTransform& cameraTransform) {
    // This function iterates over renderables and draws only those which match camera.cullingMask
    // we reuse your two loops: shapes and textures

    // SHAPES
    for (auto &entity : this->_registry->view<Hylozoa::Components::Rendering::Renderable, Hylozoa::Components::Rendering::RenderableShape, Hylozoa::Components::WorldTransform>()) {
        const Hylozoa::Components::Rendering::Renderable &renderable = this->_registry->get<Hylozoa::Components::Rendering::Renderable>(entity);
        const Hylozoa::Components::Rendering::RenderableShape &shape = this->_registry->get<Hylozoa::Components::Rendering::RenderableShape>(entity);
        const Hylozoa::Components::WorldTransform &worldTransform = this->_registry->get<Hylozoa::Components::WorldTransform>(entity);

        // culling by layer mask
        if ((renderable.layer & camera.cullingMask) == 0) continue;

        renderShape(worldTransform, renderable, shape, camera, cameraTransform);
    }

    // TEXTURES
    for (auto &entity : this->_registry->view<Hylozoa::Components::Rendering::Renderable, Hylozoa::Components::Rendering::RenderableTexture, Hylozoa::Components::WorldTransform>()) {
        const Hylozoa::Components::Rendering::Renderable &renderable = this->_registry->get<Hylozoa::Components::Rendering::Renderable>(entity);
        Hylozoa::Components::Rendering::RenderableTexture &texture = this->_registry->get<Hylozoa::Components::Rendering::RenderableTexture>(entity); // non-const because getSDLRect may modify
        const Hylozoa::Components::WorldTransform &worldTransform = this->_registry->get<Hylozoa::Components::WorldTransform>(entity);
...
void Renderer::renderShapeCircle(
    ...
    // If camera is not UI, we need to scale radius by camera.zoom and transform.scale too
    float finalRadius = radius * transform.scale.x * camera.zoom * sprite.scale; // assume uniform scale.x ~ scale.y // idk how to handle non-uniform scaling for circles
...
void Renderer::renderShapeRectangle(
    ...
    fillRect.h = rectSpecs.height * transform.scale.y * sprite.scale * camera.zoom;
...
/*
 * @brief Converts world coordinates to view (screen) coordinates based on the camera.
 *
 * This function transforms a position in world space to screen space, taking into
 * account the camera's position, zoom level, and viewport size. If the camera is
 * designated as a UI camera, the world position is treated as already being in
 * screen coordinates.
 *
...
**/
glm::vec2 Renderer::worldToView(const glm::vec2& worldPos, const Hylozoa::Components::Camera& camera, const Hylozoa::Components::WorldTransform& cameraTransform) {
    // UI camera: treat worldPos as already screen coords
    if (camera.isUI) {
        return worldPos;
    }

    // Normal camera: center camera on its transform.position, apply zoom, then center to viewport
