...
void AudioSystem::onNoiseEvent(const Components::HylozoaInternal::OnNoiseEvent &event) {
    auto entity = Entity::fromHandle(event.source, _registry);

    auto listenerView = _registry.view<Components::NoiseListener, Components::WorldTransform>();

    for (auto listenerEntity : listenerView) {
        auto &listener = listenerView.get<Components::NoiseListener>(listenerEntity);
        auto &listenerTransform = listenerView.get<Components::WorldTransform>(listenerEntity);
