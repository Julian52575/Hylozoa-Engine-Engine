...
void parent_child_system(entt::registry& registry)
{
    ...
    for (auto entity : child_view) {
        ...
        if (registry.valid(parent_comp.entity)) {
            auto& child_comp = registry.get_or_emplace<HylozoaInternal::Children>(parent_comp.entity);
...
void local_to_world_system(entt::registry& registry)
{
    ...
    auto compute_world_matrix = [&](auto& self, entt::entity e) -> matrix3x3 {
        if (visited.find(e) != visited.end()) {
            auto& local_to_world_comp = registry.get<HylozoaInternal::LocalToWorld>(e);
         ...
         if (view.contains(e)) {
            ...
            local_matrix = matrix3x3::from_transform(local_transform_comp);
        }
